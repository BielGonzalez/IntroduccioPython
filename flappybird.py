...
def crear_moneda():
    posicion_moneda = random.choice(opciones_altura_monedas)
    moneda_prueba = imagen_moneda.get_rect(midbottom =(1000,posicion_moneda))
    return moneda_prueba

def mover_moneda(coins):
    for coin in coins:
        coin.centerx -= 5
    return coins
def dibujar_monedas(coins):
    for coin in coins:
        pantalla.blit(moneda_prueba,coin)
def crear_obstaculo():
